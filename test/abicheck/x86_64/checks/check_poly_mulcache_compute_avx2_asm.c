...
int check_poly_mulcache_compute_avx2_asm(void)
{
  ...
  int violations;
  MLK_ALIGN uint8_t buf_rdi[256];  /* Output mulcache (128 x int16_t) */
  MLK_ALIGN uint8_t buf_rdx[1248]; /* Precomputed constants (624 x int16_t) */
  MLK_ALIGN uint8_t buf_rsi[512];  /* Input polynomial (256 x int16_t) */

  if (!mlk_sys_check_capability(MLK_SYS_CAP_AVX2))
  {
    fprintf(stderr,
            "ABI check poly_mulcache_compute_avx2_asm: host lacks AVX2, "
            "skipping\n");
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    randombytes(buf_rdi, 256);
    randombytes(buf_rdx, 1248);
    ...
    asm_call_stub_x86_64_sysv(
        ...
        (MLK_SYSV_ABI
         void (*)(void))mlk_poly_mulcache_compute_avx2_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for poly_mulcache_compute_avx2_asm (iteration "
              "%d): %d violations\n",
