...
int check_reduce_avx2_asm(void)
{
  ...
  {
    fprintf(stderr, "ABI check reduce_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    if (violations > 0)
    {
      fprintf(
          stderr,
          "ABI test FAILED for reduce_avx2_asm (iteration %d): %d violations\n",
