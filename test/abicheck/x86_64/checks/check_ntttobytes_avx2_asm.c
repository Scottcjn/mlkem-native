...
int check_ntttobytes_avx2_asm(void)
{
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    asm_call_stub_x86_64_sysv(
        ...
        (MLK_SYSV_ABI
         void (*)(void))mlk_ntttobytes_avx2_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for ntttobytes_avx2_asm (iteration %d): %d "
              "violations\n",
