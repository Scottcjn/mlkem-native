...
MLK_SYSV_ABI
void mlk_poly_compress_d5_avx2_asm(uint8_t *r, const int16_t *a,
                                   const uint8_t *data);
...
int check_poly_compress_d5_avx2_asm(void)
{
  ...
  int violations;
  MLK_ALIGN uint8_t buf_rdi[160]; /* Output compressed polynomial */
  ...
  {
    fprintf(stderr,
            "ABI check poly_compress_d5_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    asm_call_stub_x86_64_sysv(
        ...
        (MLK_SYSV_ABI
         void (*)(void))mlk_poly_compress_d5_avx2_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for poly_compress_d5_avx2_asm (iteration %d): "
              "%d violations\n",
