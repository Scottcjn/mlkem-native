...
MLK_SYSV_ABI
void mlk_poly_decompress_d11_avx2_asm(int16_t *r, const uint8_t *a,
                                      const uint8_t *data);
...
int check_poly_decompress_d11_avx2_asm(void)
{
  ...
  MLK_ALIGN uint8_t buf_rdx[128]; /* Precomputed decompression constants */
  MLK_ALIGN uint8_t buf_rsi[352]; /* Input compressed polynomial */

  if (!mlk_sys_check_capability(MLK_SYS_CAP_AVX2))
  {
    fprintf(
        stderr,
        "ABI check poly_decompress_d11_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    randombytes(buf_rdx, 128);
    randombytes(buf_rsi, 352);
    ...
    asm_call_stub_x86_64_sysv(
        ...
        (MLK_SYSV_ABI
         void (*)(void))mlk_poly_decompress_d11_avx2_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for poly_decompress_d11_avx2_asm (iteration "
              "%d): %d violations\n",
