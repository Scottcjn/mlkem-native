...
int check_polyvec_basemul_acc_montgomery_cached_k2_avx2_asm(void)
{
  ...
  int violations;
  MLK_ALIGN uint8_t buf_rcx[512];  /* Mulcache for b (2 x 128 x int16_t) */
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    randombytes(buf_rcx, 512);
    randombytes(buf_rdi, 512);
    randombytes(buf_rdx, 1024);
    ...
    asm_call_stub_x86_64_sysv(
        ...
        (MLK_SYSV_ABI
         void (*)(void))mlk_polyvec_basemul_acc_montgomery_cached_k2_avx2_asm);
    ...
    if (violations > 0)
    {
      fprintf(stderr,
              "ABI test FAILED for "
              "polyvec_basemul_acc_montgomery_cached_k2_avx2_asm (iteration "
              "%d): %d violations\n",
