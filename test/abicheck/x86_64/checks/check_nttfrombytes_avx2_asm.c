...
int check_nttfrombytes_avx2_asm(void)
{
  ...
  MLK_ALIGN uint8_t buf_rsi[384]; /* Input byte array (MLKEM_POLYBYTES = 384) */

  if (!mlk_sys_check_capability(MLK_SYS_CAP_AVX2))
  {
    fprintf(stderr,
            "ABI check nttfrombytes_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    asm_call_stub_x86_64_sysv(
        ...
        (MLK_SYSV_ABI
         void (*)(void))mlk_nttfrombytes_avx2_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for nttfrombytes_avx2_asm (iteration %d): %d "
              "violations\n",
