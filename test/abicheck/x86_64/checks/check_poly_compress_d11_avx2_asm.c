...
MLK_SYSV_ABI
void mlk_poly_compress_d11_avx2_asm(uint8_t *r, const int16_t *a,
                                    const uint8_t *data);
...
int check_poly_compress_d11_avx2_asm(void)
{
  ...
  int violations;
  MLK_ALIGN uint8_t buf_rdi[352]; /* Output compressed polynomial */
  MLK_ALIGN uint8_t buf_rdx[64];  /* Precomputed compression constants */
  ...
  if (!mlk_sys_check_capability(MLK_SYS_CAP_AVX2))
  {
    fprintf(
        stderr,
        "ABI check poly_compress_d11_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    randombytes(buf_rdi, 352);
    randombytes(buf_rdx, 64);
    ...
    asm_call_stub_x86_64_sysv(
        ...
        (MLK_SYSV_ABI
         void (*)(void))mlk_poly_compress_d11_avx2_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for poly_compress_d11_avx2_asm (iteration %d): "
              "%d violations\n",
